...
export function useLiveRefresh(refresh: () => void, onChange?: (change: LiveChange) => void) {
  ...
  useEffect(() => {
    ...
    for (const table of ['matches','match_events','categories','teams','matchdays','players','match_players','match_prechecks','match_foul_counters','penalty_shootouts','penalty_shootout_attempts']) {
