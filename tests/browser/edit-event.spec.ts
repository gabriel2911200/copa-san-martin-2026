...
for(const type of ['GOAL','YELLOW_CARD','RED_CARD'] as const) test(`Control: editar y anular ${type} sin cambiar tiempo ni equipo`,async({page})=>{
  ...
  const data={match:{id:'m',home_team_id:'a',away_team_id:'b',status:'PRIMER_TIEMPO',stage:'REGULAR',phase_started_at:time,phase_elapsed_seconds:120,updated_at:time},home:'Local',away:'Visitante',category:'Varones',matchday:1,server_now:time,score:{home:type==='GOAL'?1:0,away:0},players,lineup:players,events:[event],goals:type==='GOAL'?[event]:[]}
  let edits=0,voids=0
  ...
  await dialog.getByRole('combobox').selectOption('r')
  ...
  if(type==='RED_CARD') {
    ...
    await selector.getByRole('button',{name:'Cancelar selección'}).click()
  }
  await timeline.getByRole('button').click()
  ...
  expect(edits).toBe(1);expect(voids).toBe(1)
})
