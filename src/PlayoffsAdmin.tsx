...
import type { Tournament, TournamentCategory, PublicMatch } from './lib/tournament'
...
function CategoryPlayoffs({category,matches,refresh}:{category:TournamentCategory;matches:PublicMatch[];refresh:()=>Promise<void>}) {
  ...
  async function run(action:'close'|'semis') {
    ...
    try {
      ...
      const result=action==='close' ? await supabase.rpc('close_regular',{p_category_id:category.id})
        : await supabase.rpc('create_semifinals',{p_category_id:category.id,p_home1:home,p_away1:away,p_home2:remaining[0]?.id,p_away2:remaining[1]?.id})
      if(result.error) throw result.error
      await refresh()
      setSuccess(action==='close'?'Fase regular cerrada.':'Semifinales creadas.')
    } catch(err) {
  ...
  return <div className="space-y-4 rounded-xl border bg-white p-4">
    ...
    {!category.regular_closed_at ? <><p>Al terminar todos los partidos regulares, cierra la fase para fijar sus cuatro clasificados.</p><button disabled={busy} className={button} onClick={()=>void run('close')}>Cerrar fase regular</button></> : <>
      <p className="font-semibold">FASE REGULAR CERRADA · 4 CLASIFICADOS</p>
      ...
      {!semis.length && <fieldset disabled={busy} className="space-y-3">
        <legend className="font-medium">CONFIGURAR SEMIFINALES · Elegir Semifinal 1</legend>
        <label className="block">Equipo A<select aria-label="Equipo A" className={`${button} w-full`} value={home} onChange={e=>setHome(e.target.value)}><option value="">Seleccionar</option>{category.qualified.filter(t=>t.id!==away).map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label>
        <label className="block">Equipo B<select aria-label="Equipo B" className={`${button} w-full`} value={away} onChange={e=>setAway(e.target.value)}><option value="">Seleccionar</option>{category.qualified.filter(t=>t.id!==home).map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label>
      ...
      {semis.length===2 && !semis.every(m=>m.resolved) && <p>Al resolverse ambas semifinales, se crearán automáticamente la final y el tercer puesto. Solo tendrás que programar fecha y hora.</p>}
    ...
    {error && <p role="alert" className="text-red-800 whitespace-pre-line">{error}</p>}
