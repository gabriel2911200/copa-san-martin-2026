...
test('AUTOGOL selecciona solo equipo, conserva reintento y actualiza estadísticas', async ({ page }) => {
  ...
  await own.click()
  const chooser = page.getByRole('dialog', { name: '¿Qué equipo cometió el autogol?' })
  await expect(chooser.getByRole('button')).toHaveText(['San Martín', 'San Judas', 'CONFIRMAR AUTOGOL', 'Cancelar'])
  await expect(page.getByRole('dialog', { name: 'Seleccionar jugador', exact: true })).toHaveCount(0)
  await chooser.getByRole('button', { name: 'San Martín', exact: true }).click()
  await expect(chooser).toContainText('1 gol para San Judas por autogol de San Martín')
  await chooser.getByRole('button', { name: 'CONFIRMAR AUTOGOL' }).click()
  ...
  await expect(page.getByLabel('Marcador')).toHaveText('0 - 1')
