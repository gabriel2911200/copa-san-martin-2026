...
-- Datos REALES: no eliminar al terminar las pruebas.
-- Verifica categorías existentes; no crea categorías ni elimina equipos.
do $$ begin
  if (select count(*) from public.categories where name in ('Varones','Mujeres'))<>2 then
    raise exception 'Faltan las categorías reales Varones y Mujeres';
  end if;
end $$;
insert into public.teams(category_id,name,active)
select c.id, roster.name,true
from (values
  ('Varones','San Martin'),('Varones','San Judas'),('Varones','San Pablo'),
  ('Varones','San Benito'),('Varones','San Jose'),('Varones','San Roque'),
  ('Varones','San Francisco'),('Varones','San Lorenzo'),('Varones','San Andrés'),
  ('Varones','San Juan Bosco'),('Varones','San Antonio'),
  ('Mujeres','Santa Gema'),('Mujeres','Santa Rosa'),('Mujeres','Santa Ana'),
  ('Mujeres','Santa Catalina'),('Mujeres','Santa Lucia'),('Mujeres','Santa Rita')
) as roster(category,name)
join public.categories c on c.name=roster.category
on conflict (category_id,name) do update set active=true
where not public.teams.active;
commit;