...
import type { MatchEvent, FoulCount } from './matchEvents'
export type PublicMatch = {
  semifinal_number?: number
  shootout?: Shootout | null
  match: ClockState & { id: string; category_id: string; home_team_id: string; away_team_id: string; stage: string; created_at: string; updated_at: string; scheduled_date?: string | null; scheduled_time?: string | null; tiebreak_winner_team_id: string | null; walkover_loser_team_id?: string | null }
  category: string; matchday: number; home: string; away: string; server_now: string
  score: { home: number; away: number }; goals: MatchEvent[]; events?: MatchEvent[]; fouls?: FoulCount[]
...
export function matchStageLabel(item: PublicMatch) {
  return item.match.stage === 'SEMIFINAL' && item.semifinal_number ? `SEMIFINAL ${item.semifinal_number}`
    : item.match.stage === 'REGULAR' ? 'FASE REGULAR' : (stageLabels[item.match.stage] ?? item.match.stage).toUpperCase()
}
export const isActive = (m: PublicMatch) => ['PRIMER_TIEMPO','DESCANSO','SEGUNDO_TIEMPO','PAUSADO','TIEMPO_MUERTO'].includes(m.match.status)
