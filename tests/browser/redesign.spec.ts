...
test('crear envía agenda individual sin escribir datos reales',async({page})=>{
  ...
  await expect(page.getByRole('button',{name:'INICIAR PARTIDO',exact:true})).toBeEnabled()
...
test('estadísticas válidas y reprogramación compartidas sin escrituras reales',async({page})=>{
  ...
  await expect(page.getByText('1T 00:39')).toBeVisible()
  await expect(page.getByText('2T 00:25')).toHaveCount(0)
